...
class PaymentStripe extends Model
{
    ...
    public static function setCustomer($token, $user)
    {
        ...
        try {
            $customer = \Stripe\Customer::create([
                'card' => $token,
                'name' => Helper::getAccountNo($user->id),
                'description' => config('const.user.title_list.'.$user->title).' '.$user->forename.' '.$user->surname
            ]);
    ...
    public static function updateCustomer($token, $user)
    {
        ...
        try {
            ...
            $card = \Stripe\Customer::createSource($user->stripe_id,['source' => $token]);
...
//dump(\Stripe\Customer::allSources($user->stripe_id));
